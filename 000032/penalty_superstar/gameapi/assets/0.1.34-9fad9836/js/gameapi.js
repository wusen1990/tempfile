...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.showAdModal = function(options, contentDiv) {
		var opts = faZepto.extend({
			...
			returnCallback: function() {},
			returnCallback2: function() {
				self.adapters.run("ads", "callback", { "adDidLoad": !!M.adDidLoad });
				M.delayNextAdModal(true);
		...
		var header = '';
		// header = (M.abtest && M.abtest_variant ? '<b>' + M.abtest_variant.toUpperCase() + "<span style='color:#ddd'>." + ((M.priceRule && M.priceRule.name) ? M.priceRule.name.substr(1, 3) : "") + "</span>)</b> " : "");
